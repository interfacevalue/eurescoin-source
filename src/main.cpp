...
#if defined(NDEBUG)
# error "EuresCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xee949d4af49227c2387fc5c65dbe5387c4ae6b203d6a0070319a311369f1aa7b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EuresCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EuresCoin Signed Message:\n";
...
{
    // EuresCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 63000 blocks
    nSubsidy >>= (nHeight / 63000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EuresCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EuresCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1522148138, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 27Mar2018 Chancellor on brink of second bailout for banks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e46839b8003425a91ecd564f768f6959b2c7b872c33ff6911681642192fca9ac6fd9fbb2076bac92f7822f74c38fadcf2f169d16cf628b6c6ffbf17bf12f6264") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522148138;
        ...
        assert(block.hashMerkleRoot == uint256("0xa5e2bea088372569035bf503890a108d6a8b1d67781e7cae0bb80127e38fa030"));
...
unsigned char pchMessageStart[4] = { 0x53, 0xfc, 0x39, 0xdd }; // EuresCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EuresCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EuresCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EuresCoinMiner(CWallet *pwallet)
{
    printf("EuresCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eurescoin-miner");
    ...
    try { loop {
        ...
        printf("Running EuresCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EuresCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EuresCoinMiner, pwallet));
}
